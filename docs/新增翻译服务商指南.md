# 新增翻译服务商指南

本文档详细说明如何在文档翻译系统中添加新的翻译服务商（Provider）。

## 📋 目录

- [系统架构概述](#系统架构概述)
- [快速开始（兼容 OpenAI API）](#快速开始兼容-openai-api)
- [完整实现（自定义 API）](#完整实现自定义-api)
- [配置说明](#配置说明)
- [前端集成](#前端集成)
- [测试验证](#测试验证)
- [常见问题](#常见问题)

---

## 系统架构概述

### Provider 层级结构

```
BaseProvider (抽象基类)
    ├── CloudProviderValidationMixin (云端 Provider 验证混入)
    ├── LocalProviderValidationMixin (本地 Provider 验证混入)
    ├── OpenAIProvider (OpenAI API 实现)
    │   ├── MimoProvider (Mimo API - 继承 OpenAI)
    │   ├── DeepSeekProvider (DeepSeek API - 继承 OpenAI)
    │   └── YourProvider (你的服务商 - 可继承 OpenAI)
    ├── OllamaProvider (Ollama 本地服务)
    └── MockProvider (测试用)
```

### 核心组件

1. **BaseProvider** - 定义统一的 Provider 接口
2. **ProviderRegistry** - Provider 注册中心，管理所有 Provider 的创建和获取
3. **ProviderConfig** - Provider 配置数据类
4. **ProviderType** - Provider 类型枚举

---

## 快速开始（兼容 OpenAI API）

如果新服务商兼容 OpenAI API（大多数现代 AI 服务商都是），只需以下 3 个简单步骤：

### 步骤 1: 添加 Provider 类型

在 `src/core/types.py` 的 `ProviderType` 枚举中添加新类型：

```python
class ProviderType(Enum):
    """Provider 类型枚举"""
    OPENAI = "openai"
    OLLAMA = "ollama"
    MOCK = "mock"
    MIMO = "mimo"
    DEEPSEEK = "deepseek"
    QWEN = "qwen"  # ✅ 新增：阿里云通义千问
```

### 步骤 2: 创建 Provider 类

创建 `src/providers/qwen/provider.py`：

```python
"""
Qwen Provider Implementation

阿里云通义千问 API Provider 实现，兼容 OpenAI API。
"""

import logging
from typing import Dict, Any, Optional, List

from providers.base import BaseProvider, ProviderConfig, ModelInfo, ModelCapability
from providers.openai.provider import OpenAIProvider
from core.types import ProviderType

logger = logging.getLogger(__name__)


class QwenProvider(OpenAIProvider):
    """
    阿里云通义千问 Provider 实现

    由于通义千问 API 兼容 OpenAI API，我们直接继承 OpenAIProvider，
    只需重写必要的方法即可。
    """

    def __init__(self, provider_config: ProviderConfig, models_config: Optional[List[str]] = None):
        """
        初始化 Qwen Provider

        Args:
            provider_config: Provider 配置（包含 API Key、Base URL 等）
            models_config: 从配置文件中读取的允许使用的模型列表
        """
        # 调用父类 OpenAIProvider 的初始化
        super().__init__(provider_config, models_config)
        logger.info(f"Qwen Provider initialized with models: {models_config}")

    def get_name(self) -> str:
        """获取 Provider 名称"""
        return "qwen"

    def get_provider_info(self) -> Dict[str, Any]:
        """获取 Provider 信息"""
        info = super().get_provider_info()
        info["provider_type"] = ProviderType.QWEN.value
        info["provider_name"] = "阿里云通义千问"
        info["provider_description"] = "阿里云大语言模型服务"
        return info

    def __repr__(self) -> str:
        return f"<QwenProvider(base_url={self.base_url}, configured={self.is_configured()})>"
```

创建 `src/providers/qwen/__init__.py`：

```python
"""
Qwen Provider Package
"""

from .provider import QwenProvider

__all__ = ['QwenProvider']
```

### 步骤 3: 注册 Provider

在 `src/providers/registry.py` 中注册新的 Provider：

#### 3.1 在 `_register_default_classes` 方法中添加：

```python
def _register_default_classes(self) -> None:
    """注册默认的 Provider 类"""
    # 延迟导入避免循环依赖
    try:
        from .openai.provider import OpenAIProvider
        self.register_provider_class(ProviderType.OPENAI, OpenAIProvider)
    except ImportError:
        logger.warning("OpenAI provider not available")

    # ... 其他 Provider 注册 ...

    # ✅ 新增：注册 Qwen Provider
    try:
        from .qwen.provider import QwenProvider
        self.register_provider_class(ProviderType.QWEN, QwenProvider)
    except ImportError:
        logger.warning("Qwen provider not available")
```

#### 3.2 在 `_get_provider_type` 方法中添加映射：

```python
def _get_provider_type(self, provider_name: str) -> Optional[ProviderType]:
    """
    根据名称获取 Provider 类型

    Args:
        provider_name: Provider 名称

    Returns:
        Provider 类型或 None
    """
    provider_mapping = {
        "openai": ProviderType.OPENAI,
        "ollama": ProviderType.OLLAMA,
        "mimo": ProviderType.MIMO,
        "deepseek": ProviderType.DEEPSEEK,
        "mock": ProviderType.MOCK,
        "qwen": ProviderType.QWEN,  # ✅ 新增
    }

    return provider_mapping.get(provider_name.lower())
```

#### 3.3 在 `get_or_create` 方法中添加模型配置：

```python
def get_or_create(self, provider_name: str, model: Optional[str] = None, config: Optional[ProviderConfig] = None) -> Optional[BaseProvider]:
    """获取或创建 Provider 实例"""
    # ... 前面的代码 ...

    if provider_type == ProviderType.OPENAI:
        models_config = global_config.openai_models
        provider = self._provider_classes[provider_type](config, models_config)
    elif provider_type == ProviderType.MIMO:
        models_config = global_config.mimo_models
        provider = self._provider_classes[provider_type](config, models_config)
    elif provider_type == ProviderType.DEEPSEEK:
        models_config = global_config.deepseek_models
        provider = self._provider_classes[provider_type](config, models_config)
    elif provider_type == ProviderType.QWEN:  # ✅ 新增
        models_config = global_config.qwen_models
        provider = self._provider_classes[provider_type](config, models_config)
    else:
        provider = self._provider_classes[provider_type](config)

    # ... 后面的代码 ...
```

### 步骤 4: 添加环境配置

在项目根目录的 `.env` 文件中添加配置：

```bash
# ==================================================
# Qwen (阿里云通义千问) Configuration
# ==================================================

# Provider 名称
LLM_PROVIDER=qwen

# API 密钥（从阿里云控制台获取）
QWEN_API_KEY=sk-your-qwen-api-key-here

# 可用模型列表
QWEN_MODELS=["qwen-turbo", "qwen-plus", "qwen-max", "qwen-max-longcontext"]

# API Base URL（通义千问兼容模式）
QWEN_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1
```

**提示**：获取 Qwen API Key 的步骤：
1. 访问 [阿里云百炼平台](https://bailian.console.aliyun.com/)
2. 创建 API-KEY
3. 复制 API Key 到 `.env` 文件

### 步骤 5: 更新配置加载逻辑

在 `src/core/config.py` 中添加配置读取：

```python
class Config:
    """全局配置管理"""

    # ... 现有属性 ...

    @property
    def qwen_api_key(self) -> str:
        """获取 Qwen API Key"""
        return os.getenv("QWEN_API_KEY", "")

    @property
    def qwen_base_url(self) -> str:
        """获取 Qwen Base URL"""
        return os.getenv("QWEN_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")

    @property
    def qwen_models(self) -> List[str]:
        """获取 Qwen 的模型列表"""
        models_str = os.getenv("QWEN_MODELS", '["qwen-turbo", "qwen-plus", "qwen-max"]')
        try:
            return json.loads(models_str)
        except json.JSONDecodeError:
            logger.warning(f"Failed to parse QWEN_MODELS: {models_str}")
            return ["qwen-turbo"]

    def get_provider_config(self, provider_name: str) -> Dict[str, Any]:
        """
        获取 Provider 配置

        Args:
            provider_name: Provider 名称

        Returns:
            Provider 配置字典
        """
        provider_name = provider_name.lower()

        # ... 其他 Provider 的配置 ...

        if provider_name == "qwen":
            return {
                "provider_type": ProviderType.QWEN,
                "api_key": self.qwen_api_key,
                "base_url": self.qwen_base_url,
                "timeout_seconds": 120,
                "max_retries": 3,
            }

        # ... 默认配置 ...
```

---

## 完整实现（自定义 API）

如果服务商不兼容 OpenAI API，需要完整实现 `BaseProvider` 的所有抽象方法。

### 示例：实现自定义 API Provider

```python
"""
Custom Provider Implementation

完全自定义 API 的 Provider 实现示例。
"""

import logging
import requests
from typing import Dict, Any, Optional, List, Tuple

from providers.base import (
    BaseProvider,
    ProviderConfig,
    ModelInfo,
    ModelCapability
)
from providers.mixins import CloudProviderValidationMixin
from core.types import ProviderType

logger = logging.getLogger(__name__)


class CustomProvider(BaseProvider, CloudProviderValidationMixin):
    """
    自定义 API Provider 实现

    实现所有必需的 BaseProvider 方法。
    """

    def __init__(self, provider_config: ProviderConfig):
        """
        初始化 Custom Provider

        Args:
            provider_config: Provider 配置
        """
        super().__init__(provider_config)
        self.api_key = provider_config.api_key
        self.base_url = provider_config.base_url
        self.session = requests.Session()
        self.session.headers.update({
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json"
        })

    # ==================================================
    # 配置验证相关方法
    # ==================================================

    def is_configured(self) -> bool:
        """
        检查 Provider 是否已配置

        Returns:
            是否已配置
        """
        return bool(self.api_key)

    def validate_configuration(self) -> Tuple[bool, Optional[str]]:
        """
        验证 Provider 配置

        Cloud Provider 专用: 检查 API Key 有效性

        Returns:
            (is_valid, error_message): 配置是否有效及错误信息
        """
        # 使用 Cloud Provider 通用校验逻辑
        return self.validate_api_key(self.config.api_key)

    def health_check(self) -> Tuple[bool, Optional[str]]:
        """
        真实 Health Check

        Cloud Provider 专用: 验证 API Key + 尝试初始化

        Returns:
            (is_healthy, error_message): 是否健康及错误信息
        """
        # 第一步: 配置验证
        is_valid, error_msg = self.validate_configuration()
        if not is_valid:
            return False, error_msg

        # 第二步: 尝试调用 API
        try:
            response = self.session.get(
                f"{self.base_url}/models",
                timeout=10
            )
            if response.status_code == 200:
                return True, None
            else:
                return False, f"API returned status {response.status_code}"
        except Exception as e:
            return False, f"Health check failed: {str(e)}"

    # ==================================================
    # 模型管理相关方法
    # ==================================================

    def get_available_models(self) -> List[str]:
        """
        获取可用模型列表

        Returns:
            模型名称列表
        """
        if not self.is_configured():
            return []

        try:
            response = self.session.get(
                f"{self.base_url}/models",
                timeout=10
            )
            if response.status_code == 200:
                data = response.json()
                # 假设 API 返回格式为 {"models": [{"id": "model-1"}, ...]}
                return [model["id"] for model in data.get("models", [])]
        except Exception as e:
            logger.error(f"Failed to get models: {e}")

        return []

    def get_model_info(self, model_name: str) -> Optional[ModelInfo]:
        """
        获取模型信息

        Args:
            model_name: 模型名称

        Returns:
            模型信息或 None
        """
        if model_name not in self.get_available_models():
            return None

        return ModelInfo(
            name=model_name,
            provider=ProviderType.CUSTOM,
            capabilities=[
                ModelCapability.TEXT_GENERATION,
                ModelCapability.TRANSLATION,
                ModelCapability.MULTILINGUAL
            ],
            max_tokens=4096,
            supports_streaming=True,
            supports_function_calling=False
        )

    # ==================================================
    # 文本生成相关方法
    # ==================================================

    async def generate_async(
        self,
        prompt: str,
        model: str,
        temperature: float = 0.7,
        max_tokens: Optional[int] = None,
        **kwargs
    ) -> str:
        """
        异步生成文本

        Args:
            prompt: 输入提示
            model: 模型名称
            temperature: 温度参数
            max_tokens: 最大生成 token 数
            **kwargs: 其他参数

        Returns:
            生成的文本
        """
        if not self.is_configured():
            raise RuntimeError("Custom provider is not configured")

        try:
            # 使用 run_in_executor 在线程池中运行同步请求
            import asyncio
            loop = asyncio.get_event_loop()
            return await loop.run_in_executor(
                None,
                self._sync_generate,
                prompt,
                model,
                temperature,
                max_tokens,
                **kwargs
            )
        except Exception as e:
            logger.error(f"Error generating text with Custom provider: {e}")
            raise

    def _sync_generate(
        self,
        prompt: str,
        model: str,
        temperature: float,
        max_tokens: Optional[int],
        **kwargs
    ) -> str:
        """
        同步生成文本（内部方法）

        Args:
            prompt: 输入提示
            model: 模型名称
            temperature: 温度参数
            max_tokens: 最大生成 token 数
            **kwargs: 其他参数

        Returns:
            生成的文本
        """
        payload = {
            "model": model,
            "messages": [{"role": "user", "content": prompt}],
            "temperature": temperature,
        }

        if max_tokens:
            payload["max_tokens"] = max_tokens

        response = self.session.post(
            f"{self.base_url}/chat/completions",
            json=payload,
            timeout=self.config.timeout_seconds
        )

        response.raise_for_status()
        data = response.json()

        # 假设 API 返回格式与 OpenAI 兼容
        return data["choices"][0]["message"]["content"]

    # ==================================================
    # 翻译相关方法
    # ==================================================

    def translate(
        self,
        text: str,
        source_lang: str = "zh",
        target_lang: str = "en",
        model: Optional[str] = None,
        **kwargs
    ) -> str:
        """
        翻译文本

        Args:
            text: 待翻译文本
            source_lang: 源语言
            target_lang: 目标语言
            model: 模型名称（可选）
            **kwargs: 其他参数

        Returns:
            翻译后的文本
        """
        if not text.strip():
            return text

        if not model:
            # 使用默认模型
            available_models = self.get_available_models()
            if not available_models:
                raise RuntimeError("No models available")
            model = available_models[0]

        # 构建翻译提示
        prompt = f"""请将以下{source_lang}文本翻译为{target_lang}：

{text}

要求：
1. 保持原文的意思和语气
2. 使用自然流畅的{target_lang}表达
3. 只返回翻译结果，不要添加解释"""

        try:
            return self.generate(prompt, model, temperature=0.3, **kwargs)
        except Exception as e:
            logger.error(f"Translation failed: {e}")
            # 返回原文本作为后备
            return text

    # ==================================================
    # 工具方法
    # ==================================================

    def get_name(self) -> str:
        """获取 Provider 名称"""
        return "custom"

    def get_provider_info(self) -> Dict[str, Any]:
        """获取 Provider 信息"""
        return {
            "provider_type": ProviderType.CUSTOM.value,
            "provider_name": "Custom Provider",
            "provider_description": "自定义 API Provider",
            "is_configured": self.is_configured(),
            "available_models": self.get_available_models(),
            "config": {
                "timeout_seconds": self.config.timeout_seconds,
                "max_retries": self.config.max_retries,
                "has_api_key": bool(self.config.api_key),
                "base_url": self.config.base_url
            }
        }

    def __repr__(self) -> str:
        return f"<CustomProvider(base_url={self.base_url}, configured={self.is_configured()})>"
```

---

## 配置说明

### 环境变量命名规范

所有服务商配置遵循以下命名规范：

```bash
# Provider 配置模板
{PROVIDER_NAME}_API_KEY=your_api_key_here
{PROVIDER_NAME}_MODELS=["model-1", "model-2", "model-3"]
{PROVIDER_NAME}_BASE_URL=https://api.provider.com/v1
```

### 支持的配置选项

| 配置项 | 类型 | 必需 | 说明 |
|--------|------|------|------|
| `api_key` | string | 是* | API 密钥（本地 Provider 不需要） |
| `base_url` | string | 是 | API 基础 URL |
| `models` | list | 是 | 支持的模型列表 |
| `timeout_seconds` | int | 否 | 请求超时时间（默认 120） |
| `max_retries` | int | 否 | 最大重试次数（默认 3） |

### 常见服务商配置示例

#### OpenAI

```bash
OPENAI_API_KEY=sk-your-openai-api-key
OPENAI_MODELS=["gpt-3.5-turbo", "gpt-4", "gpt-4-turbo", "gpt-4o"]
OPENAI_BASE_URL=https://api.openai.com/v1
```

#### 阿里云通义千问 (Qwen)

```bash
QWEN_API_KEY=sk-your-qwen-api-key
QWEN_MODELS=["qwen-turbo", "qwen-plus", "qwen-max", "qwen-max-longcontext"]
QWEN_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1
```

#### Moonshot (月之暗面)

```bash
MOONSHOT_API_KEY=sk-your-moonshot-api-key
MOONSHOT_MODELS=["moonshot-v1-8k", "moonshot-v1-32k", "moonshot-v1-128k"]
MOONSHOT_BASE_URL=https://api.moonshot.cn/v1
```

#### 智谱 AI (ChatGLM)

```bash
CHATGLM_API_KEY=your-chatglm-api-key
CHATGLM_MODELS=["glm-4", "glm-4-flash", "glm-4-air", "glm-3-turbo"]
CHATGLM_BASE_URL=https://open.bigmodel.cn/api/paas/v4
```

---

## 前端集成

### 更新 TypeScript 类型定义

在 `frontend/src/types/api.ts` 中更新 provider 类型：

```typescript
/**
 * Provider 名称类型
 */
export type ProviderName =
  | "openai"
  | "ollama"
  | "mimo"
  | "deepseek"
  | "mock"
  | "qwen";  // ✅ 新增

/**
 * 翻译请求接口
 */
export interface TranslationRequest {
  source_markdown: string;
  glossary?: Record<string, string>;
  llm_config?: {
    provider: ProviderName;  // 会自动包含新类型
    model: string;
    temperature?: number;
    extra_options?: Record<string, any>;
  };
}
```

### 更新 Provider 选择器

在 `frontend/src/components/ProviderSelector/index.tsx` 中添加新服务商信息：

```typescript
const providerInfo: Record<string, { name: string; description: string; color: string }> = {
  openai: {
    name: 'OpenAI兼容',
    description: '兼容OpenAI API的LLM模型，适合专业翻译',
    color: '#10a37f'
  },
  ollama: {
    name: 'Ollama',
    description: '本地运行的开源模型，保护隐私',
    color: '#ff6b35'
  },
  mimo: {
    name: 'Mimo',
    description: '小米Mimo AI模型，高效快速',
    color: '#FF6900'
  },
  deepseek: {
    name: 'DeepSeek',
    description: 'DeepSeek推理模型，擅长复杂逻辑',
    color: '#4D6BFE'
  },
  qwen: {  // ✅ 新增
    name: '通义千问',
    description: '阿里云大语言模型服务，支持多种场景',
    color: '#FF6A00'
  }
};
```

---

## 测试验证

### 1. 单元测试

创建 `tests/providers/test_qwen_provider.py`：

```python
"""
Qwen Provider 单元测试
"""

import pytest
from providers.qwen.provider import QwenProvider
from providers.base import ProviderConfig
from core.types import ProviderType


def test_qwen_provider_initialization():
    """测试 Qwen Provider 初始化"""
    config = ProviderConfig(
        provider_type=ProviderType.QWEN,
        api_key="test-key",
        base_url="https://dashscope.aliyuncs.com/compatible-mode/v1"
    )

    provider = QwenProvider(config, ["qwen-turbo", "qwen-plus"])

    assert provider.get_name() == "qwen"
    assert provider.is_configured()


def test_qwen_provider_models():
    """测试获取可用模型"""
    config = ProviderConfig(
        provider_type=ProviderType.QWEN,
        api_key="test-key",
        base_url="https://dashscope.aliyuncs.com/compatible-mode/v1"
    )

    provider = QwenProvider(config, ["qwen-turbo", "qwen-plus", "qwen-max"])

    models = provider.get_available_models()
    assert "qwen-turbo" in models
    assert "qwen-plus" in models
    assert "qwen-max" in models
    assert len(models) == 3
```

运行测试：

```bash
pytest tests/providers/test_qwen_provider.py -v
```

### 2. 集成测试

#### 检查 Provider 状态

```bash
curl http://localhost:8000/api/v1/providers/qwen/status
```

预期响应：

```json
{
  "success": true,
  "provider": "qwen",
  "available": true,
  "models": ["qwen-turbo", "qwen-plus", "qwen-max"]
}
```

#### 测试翻译功能

```bash
curl -X POST "http://localhost:8000/api/v1/translate" \
  -H "Content-Type: application/json" \
  -d '{
    "source_markdown": "# 测试标题\n这是测试内容",
    "llm_config": {
      "provider": "qwen",
      "model": "qwen-turbo",
      "temperature": 0.3
    }
  }'
```

### 3. 前端测试

1. 启动前端服务：
```bash
cd frontend
npm run dev
```

2. 打开浏览器访问 `http://localhost:5173`

3. 在 Provider 选择器中验证：
   - 能否看到新添加的 Provider
   - 能否选择该 Provider 的模型
   - 能否正常上传文件并翻译

---

## 常见问题

### Q1: Provider 显示"不可用"

**可能原因**：
1. API Key 未配置或配置错误
2. Base URL 配置错误
3. 网络连接问题

**解决方法**：
```bash
# 检查环境变量
echo $QWEN_API_KEY
echo $QWEN_BASE_URL

# 检查 API 连通性
curl -H "Authorization: Bearer $QWEN_API_KEY" \
  https://dashscope.aliyuncs.com/compatible-mode/v1/models
```

### Q2: 翻译失败，提示"Provider not found"

**可能原因**：
1. Provider 未在 `registry.py` 中注册
2. Provider 类型枚举未添加
3. Provider 类导入失败

**解决方法**：
```python
# 检查 registry.py
from providers.qwen.provider import QwenProvider  # 是否能正常导入

# 检查 ProviderType
ProviderType.QWEN.value  # 应该返回 "qwen"

# 检查映射
provider_mapping.get("qwen")  # 应该返回 ProviderType.QWEN
```

### Q3: 模型列表为空

**可能原因**：
1. `MODELS` 环境变量未配置或格式错误
2. 配置加载逻辑有问题

**解决方法**：
```bash
# 检查环境变量（注意 JSON 格式）
echo $QWEN_MODELS
# 正确格式：["qwen-turbo", "qwen-plus"]
# 错误格式：qwen-turbo,qwen-plus（缺少 JSON 格式）

# 测试解析
python3 -c "import json, os; print(json.loads(os.getenv('QWEN_MODELS')))"
```

### Q4: 翻译结果包含中文（未翻译）

**可能原因**：
1. 使用了推理模型（如 DeepSeek-R1）的推理内容
2. 模型输出包含 `reasoning_content`

**解决方法**：
`OpenAIProvider` 已经自动处理了推理模型，会丢弃 `reasoning_content`。如果是自定义实现，需要添加：

```python
if hasattr(message, 'reasoning_content') and message.reasoning_content:
    logger.warning("Discarding reasoning content")
    # 只返回实际内容，不返回推理过程
    return message.content
```

### Q5: 如何添加本地 Provider（如 Ollama）

本地 Provider 不需要 API Key，需要使用 `LocalProviderValidationMixin`：

```python
from providers.mixins import LocalProviderValidationMixin

class LocalProvider(BaseProvider, LocalProviderValidationMixin):
    def validate_configuration(self) -> tuple[bool, Optional[str]]:
        """本地 Provider：检查服务是否运行"""
        # 检查本地服务端口
        # ...
        return True, None
```

---

## 总结

### 快速参考卡片

| 任务 | 文件 | 代码量 |
|------|------|--------|
| 添加类型枚举 | `src/core/types.py` | 1 行 |
| 创建 Provider 类 | `src/providers/{name}/provider.py` | 20-50 行 |
| 注册 Provider | `src/providers/registry.py` | 3 处修改 |
| 环境配置 | `.env` | 3-4 行 |
| 前端类型 | `frontend/src/types/api.ts` | 1 行 |
| 前端信息 | `frontend/src/components/ProviderSelector/index.tsx` | 4 行 |

**总计**：兼容 OpenAI API 的服务商，约 **30-60 行代码** 即可完成集成。

### 最佳实践

1. **优先继承**：如果服务商兼容 OpenAI API，直接继承 `OpenAIProvider`
2. **类型安全**：使用 `ProviderType` 枚举，避免字符串硬编码
3. **配置分离**：所有配置通过环境变量管理，不硬编码
4. **错误处理**：实现 `validate_configuration` 和 `health_check`
5. **日志记录**：使用 `logger` 记录关键操作和错误
6. **测试覆盖**：编写单元测试和集成测试

---

## 附录

### A. 完整文件清单

添加新 Provider 需要修改/创建的文件：

```
document-translation/
├── src/
│   ├── core/
│   │   └── types.py                          # ✏️ 修改：添加 ProviderType 枚举
│   ├── providers/
│   │   ├── {your_provider}/                  # ✨ 新建：Provider 目录
│   │   │   ├── __init__.py                   # ✨ 新建
│   │   │   └── provider.py                   # ✨ 新建：Provider 实现
│   │   └── registry.py                       # ✏️ 修改：注册 Provider
│   └── core/
│       └── config.py                         # ✏️ 修改：添加配置读取
├── frontend/
│   └── src/
│       ├── types/
│       │   └── api.ts                        # ✏️ 修改：更新类型定义
│       └── components/
│           └── ProviderSelector/
│               └── index.tsx                 # ✏️ 修改：添加 Provider 信息
├── .env                                       # ✏️ 修改：添加环境变量
└── docs/
    └── ADD_NEW_PROVIDER_GUIDE.md             # 📖 本文档
```

### B. 相关文档

- [系统架构指南](ARCHITECTURE_GUIDE.md)
- [API 规范文档](RESTAPI规范.md)
- [Provider 接口定义](../src/providers/base.py)

### C. 获取帮助

如果遇到问题：

1. 查看现有 Provider 实现（`src/providers/`）
2. 阅读本文档的[常见问题](#常见问题)部分
3. 检查日志输出（后端和浏览器控制台）
4. 运行测试验证配置

---

**文档版本**: 1.0.0
**最后更新**: 2025-01-19
**维护者**: Document Translation Team
