...
阿里云通义千问 API Provider 实现，兼容 OpenAI API。
"""
...
class QwenProvider(OpenAIProvider):
    ...
    由于通义千问 API 兼容 OpenAI API，我们直接继承 OpenAIProvider，
    只需重写必要的方法即可。
    """
    ...
    def __init__(self, provider_config: ProviderConfig, models_config: Optional[List[str]] = None):
        ...
        Args:
            provider_config: Provider 配置（包含 API Key、Base URL 等）
            models_config: 从配置文件中读取的允许使用的模型列表
        """
        # 调用父类 OpenAIProvider 的初始化
    ...
    def get_provider_info(self) -> Dict[str, Any]:
        ...
        info["provider_name"] = "阿里云通义千问"
        info["provider_description"] = "阿里云大语言模型服务"
        return info
